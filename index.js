...
async function syncOnce() {
  ...
  try {
    ...
    // Se eliminó el DELETE automático para no perder los datos si la app se reinicia
    ...
    while (keepFetching) {
      ...
      if (chunk.length === 0) {
        console.log('[sync] Finalizó la descarga. No hay más datos por procesar.');
        keepFetching = false;
        break;
      }
      ...
      if (batchMaxTs <= lastTs) {
        console.log(`[sync] Paginación finalizada en ts=${batchMaxTs}`);
        keepFetching = false;
        break;
      }
      ...
      console.log(`[sync] Lote guardado (${chunk.length} items). ts actual: ${lastTs}`);
